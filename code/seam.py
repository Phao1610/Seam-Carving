...
class SeamCarving: 
    ...
    @jit
    def remove_object(self, removal_mask,protect_mask):
        self.mask = removal_mask.copy()
        self.protect = protect_mask.copy()
        ...
        if rotate_flag: 
            ...
            self.protect = imutils.rotate_bound(self.protect, angle=90)

            
        for step in tqdm(range(dmask), desc='Removing Object'):
            emap = self.gen_emap()
            emap = np.where((self.protect==255),1000,emap)

            emap = np.where((self.mask==255), -1000, emap)
            seam = self.get_minimum_seam(emap)
            
            h, w = self.mask.shape 
            ht, wt = self.protect.shape   

            new_mask = np.zeros(shape=(h, w-1))
            new_protect = np.zeros(shape=(ht,wt-1))

            for row in range(0, h):
                col = seam[row]
                new_mask[row, :col] = self.mask[row, :col]
                new_mask[row, col:] = self.mask[row, col+1:]
                new_protect[row, :col] = self.protect[row, :col]
                new_protect[row, col:] = self.protect[row,col+1:]                
            self.mask = new_mask
        ...
        temp_img = self.new_img.copy()
        cv2.imwrite('anh_rie.png',self.new_img)
        cv2.imwrite('anh.png',self.protect)
        ...
        cv2.imwrite('anh_riew.png',self.new_img)
        return self.new_img.copy()
